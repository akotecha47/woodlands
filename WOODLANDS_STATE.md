...
**`061` — WRITTEN AND APPLIED 18 August, NOT REBUILD-PROVEN. This is now the outstanding proof**, owed before any `062` is written. It was written on a proven base, which is what the gate requires. A rebuild proof expires: it is a claim about a file range on a date, not a permanent property of the project. Code-only and data-op changes do not touch this clock.

**`061` gives that run two things to check:** a **second** `security_invoker` view (`v_fm_attendance`) plus two **`STABLE`/`IMMUTABLE` SQL functions it depends on** — a rebuild must reproduce the EXECUTE grants on `fm_market_day`/`fm_last_n_market_days` or the view breaks for `authenticated` — and a **second migration-borne reference seed** (6 categories / 17 product types / 51 items / 6 fees, which WILL exist on a rebuild; the Farmers Market demo data in `data-ops/008` deliberately will NOT, so expect the `fm_holders`/`fm_visits`/`fm_waiting_list` counts to diverge and nothing else).
