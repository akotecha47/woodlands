...
**Now owed: `059` has not been rebuild-proven.** The next migration written triggers a full throwaway rebuild of `001`–`059` diffed against production. **Before the next migration is written, not after.** A rebuild proof expires — it is a claim about a file range on a date, not a permanent property of the project. Code-only and data-op changes do not touch this clock.
